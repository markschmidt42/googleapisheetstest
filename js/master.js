...
googleAuth.handleAuthResult = function(authResult) {
...
googleAuth.handleAuthClick = function(event) {
    gapi.auth.authorize(
      {client_id: CLIENT_ID, scope: SCOPES, immediate: false},
      googleAuth.handleAuthResult
    );
    return false;
}

// Check if current user has authorized this application.
googleAuth.checkAuth = function() {
  gapi.auth.authorize(
  {
    'client_id': CLIENT_ID,
    'scope': SCOPES.join(' '),
...
$(function() { 
  $("#authorize-button").on('click', googleAuth.handleAuthClick);
})
...
$(function() {
    ...
    else 
    {
      ...
      function loadSheetsApi() {
        ...
        gapi.client.load(discoveryUrl).then(useSheetsApi);
      }
